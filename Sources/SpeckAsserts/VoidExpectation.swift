...
public class VoidExpectation: BasicExpectation {
  ...
  public func to<T: Equatable>(
    change value: @autoclosure () throws -> T?,
    ...
    do {
      ...
      self.assert(
        ...
        msg: """
        change value from \(String(describing: expectedFromValue)) \
  ...
  public func to<T: Equatable>(
    ...
    do {
      ...
      self.assert(
        value == newValue,
        msg: "change value to \(String(describing: newValue)), got \(String(describing: value))"
      )
    } catch let err {
      self.assert(false, msg: "error: \(err)")
    }
